...
import System, os, clr, sys
from System import Int32 as intG # change for ironpython3
...
from Microsoft.ML.Probabilistic.Distributions import Discrete
...
class CreateArray : 
    ...
    def create_var_arr_of_arr(cls, jagged_lengths, type):
        ...
        sizes = System.Array.CreateInstance(intG, length)#
        for i in range(0, length):
            sizes[i] = jagged_lengths[i] 
        outer = Range(length)
        # make a constant VariableArray from the length array
        sizes_var = Variable.Constant[intG](sizes, outer)
    ...
    def create_var_2D_arr_of_arr(cls, jagged_2D_lengths, type):
        ...
        len_array = System.Array.CreateInstance(intG, length1, length2)
        ...
        sizes_var = Variable.Constant[intG](len_array, outer1, outer2)
    ...
    def create_var_arr_of_2D_arr(cls, jagged_lengths2D, type):
        ...
        outer = Range(length)
        sizes1 = System.Array.CreateInstance(intG, length)
        for i in range(0, length):
            sizes1[i] = jagged_lengths2D[i][0] 
        sizes2 = System.Array.CreateInstance(intG, length)
        for i in range(0, length):
            sizes2[i] = jagged_lengths2D[i][1] 
        # make constant VariableArrays from the length arrays
        sizes1_var = Variable.Constant[intG](sizes1, outer)
        sizes2_var = Variable.Constant[intG](sizes2, outer)
