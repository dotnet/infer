...
namespace Microsoft.ML.Probabilistic.Learners.BayesPointMachineClassifierInternal
{
	public partial class CompoundSparseMulticlassBpmTrainingEvidence_EP : IGeneratedAlgorithm
	{
		...
		private void Changed_ClassCount_FeatureCount_FeatureIndexes_FeatureValues_InstanceCount_InstanceFeatureCounts_Lab22(int numberOfIterations)
		{
			...
			this.ModelSelector_marginal_F = VariableOp.MarginalAverageConditional<Bernoulli>(this.ModelSelector_selector_B, this.vBernoulli9, this.ModelSelector_marginal_F);
			for(int FeatureRange = 0; FeatureRange<this.FeatureCount; FeatureRange++) {
				for(int ClassRange = 0; ClassRange<this.ClassCount; ClassRange++) {
					...
					for(int ClassMaxNoisyScore = 0; ClassMaxNoisyScore<this.ClassCount; ClassMaxNoisyScore++) {
						if (this.Labels[InstanceRange]==ClassMaxNoisyScore) {
							if (ClassMaxNoisyScore!=ClassRange) {
								...
								this.ModelSelector_selector_cases_0_rep40_rep_B[InstanceRange][ClassRange] = Bernoulli.FromLogOdds(ConstrainEqualOp<bool>.LogEvidenceRatio(true, this.vbool76[InstanceRange][ClassMaxNoisyScore][ClassRange]));
							}
						}
					}
				}
		...
		private void Constant()
		{
			...
			this.vBernoulli9 = Bernoulli.Uniform();
