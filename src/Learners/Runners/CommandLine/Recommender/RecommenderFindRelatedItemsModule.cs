    ...
    internal class RecommenderFindRelatedItemsModule : CommandLineModule
    {
        ...
        public override bool Run(string[] args, string usagePrefix)
        {
            ...
            var trainedModel = MatchboxRecommender.Load<RecommenderDataset, int, User, Item, int, RatingDistribution, DummyFeatureSource>(
                ...
                writeUser: (writer, x) => writer.Write(x.ToString()),
                writeItem: (writer, x) => writer.Write(x.ToString()));
            var evaluator = new RecommenderEvaluator<RecommenderDataset, User, Item, int, int, RatingDistribution>(
                RecommenderMappings.StarRatingRecommender.ForEvaluation());
