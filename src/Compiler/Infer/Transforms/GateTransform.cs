...
#pragma warning disable 162
        ...
        protected override IStatement DoConvertStatement(IStatement ist)
        {
            ...
            if (condInfo != null)
            {
                if (false)
                {
                    if (conditionContext.Count > 0 && condInfo == conditionContext[conditionContext.Count - 1])
                    {
                        // this statement was generated by the current ConditionInformation.  Temporarily remove the ConditionInformation from the context, and restore later.
                        conditionContext.RemoveAt(conditionContext.Count - 1);
                    }
                    else
                    {
                        condInfo = null;
                ...
                if (caseNumber is ILiteralExpression)
                {
                    ...
                    if (!casesVars.TryGetValue(caseNumber, out casesVar))
                    {
                        ...
                        IExpression copy = Builder.StaticGenericMethod(
                            new Func<PlaceHolder, PlaceHolder>(Factor.Copy<PlaceHolder>), new Type[] {typeof (bool)}, item);
        ...
        internal IVariableReferenceExpression GetCloneForCase(BasicTransformContext context, ConditionInformation condInfo, bool isDef, IExpression caseNumber)
        {
            ...
            if (isDef)
            {
                ...
                IExpression copyExpr = Builder.StaticGenericMethod(
                    new Func<PlaceHolder, PlaceHolder>(Factor.Copy), new Type[] {exprType}, caseVarRef);
            ...
            else
            {
                ...
                IExpression copyExpr = Builder.StaticGenericMethod(
                    new Func<PlaceHolder, PlaceHolder>(Factor.Copy), new Type[] {exprType}, item);
