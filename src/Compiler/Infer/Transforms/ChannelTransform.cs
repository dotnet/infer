        ...
        protected override IExpression ConvertVariableDeclExpr(IVariableDeclarationExpression ivde)
        {
            ...
            if (isInferred || (isStochastic && !suppressVariableFactor))
            {
                ...
                if (algorithm is GibbsSampling && ((GibbsSampling)algorithm).UseSideChannels)
                {
                    ...
                    if (isDerived)
                    {
                        Delegate d = new FuncOut3<PlaceHolder, int, int, int, PlaceHolder, PlaceHolder, PlaceHolder, PlaceHolder[]>(VariableFactor.ReplicateWithMarginalGibbs);
                    ...
                    {
                        Delegate d = new FuncOut3<PlaceHolder, int, int, int, PlaceHolder, PlaceHolder, PlaceHolder, PlaceHolder[]>(VariableFactor.UsesEqualDefGibbs);
                        variableFactorExpr = Builder.StaticGenericMethod(d, genArgs, defExpr, countExpr, burnInExpr, thinExpr, marginalExpr, samplesExpr, conditionalsExpr);
                    }
                }
                else
                {
                    ...
                    if (isDerived)
                    {
                        d = new FuncOut<PlaceHolder, int, PlaceHolder, PlaceHolder[]>(VariableFactor.ReplicateWithMarginal<PlaceHolder>);
                    }
                    else
                    {
                        d = new FuncOut<PlaceHolder, int, PlaceHolder, PlaceHolder[]>(VariableFactor.UsesEqualDef<PlaceHolder>);
                    }
        ...
        private object AddMarginalStatements(IExpression expr)
        {
            ...
            if (!context.InputAttributes.Has<VariableToChannelInformation>(decl))
            {
                ...
                else
                {
                    ...
                    Delegate d = new FuncOut<PlaceHolder, int, PlaceHolder, PlaceHolder[]>(VariableFactor.ReplicateWithMarginal);
